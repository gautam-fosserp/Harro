...
frappe.ui.form.on("Travel Hotel Booking", {
    refresh(frm) {
        travel_segment_add_back_button(frm);
    },
    custom_create_purchase_invoice_hotel(frm) {
        const create_invoice = () => {
            frappe.call({
                method: "harro.harro.doctype.travel_planning.travel_planning.get_hotel_segment_purchase_invoice_defaults",
    ...
    custom_send_email_hotel(frm) {
        if (frm.doc.custom_hotel_email_sent) {
            frappe.msgprint("Email already sent for this record.");
            return;
        }
        frappe.call({
            method: "harro.harro.doctype.travel_planning.travel_planning.send_hotel_segment_email",
            args: { name: frm.doc.name },
            callback(r) {
                if (!r.exc) {
